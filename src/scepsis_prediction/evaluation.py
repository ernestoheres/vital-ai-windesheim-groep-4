...
- `testset (with label).csv`:
  - Bevat voor elke patient de referentie sepsislabels (0 = geen sepsis, 1 = sepsis).
  - Kolommen (ten minste): `Patient_ID`, `SepsisLabel`.

  - `predictions.csv`:
  - Bevat jullie voorspellingen (0 = geen sepsis, 1 = sepsis) voor elke patient.
  - Kolommen (ten minste): `Patient_ID`, `SepsisLabel`.
...
def evaluate_sepsis_score(
    ...
    for patient_id in label_patients:
        labels = labels_df.loc[
            labels_df[patient_col] == patient_id, label_header
        ].to_numpy(dtype=float)
        predictions = preds_df.loc[
            preds_df[patient_col] == patient_id, prediction_header
        ].to_numpy(dtype=float)

        if len(labels) != len(predictions):
            raise Exception(
                f"Row counts for patient {patient_id} differ between label and prediction CSVs."
            )
        ...
        for i in range(num_rows):
            ...
            if predictions[i] not in (0, 1):
                raise Exception(
                    f"Prediction must be 0 or 1 (patient {patient_id}, row {i})."
                )
    ...
    for k in range(num_files):
        ...
        if np.any(labels):
            t_sepsis = np.argmax(labels) - dt_optimal
            best_predictions[
                max(0, t_sepsis + dt_early) : min(t_sepsis + dt_late + 1, num_rows)
            ] = 1
