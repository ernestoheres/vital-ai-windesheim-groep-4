    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        baseEnvSetup = pkgs: ''
          ...
          # Repair SSH agent socket in Nix shells when inherited value is broken.
          runtime_dir="''${XDG_RUNTIME_DIR:-/run/user/$(id -u)}"
          if [ -z "''${SSH_AUTH_SOCK:-}" ] || [ ! -S "''${SSH_AUTH_SOCK}" ]; then
            if [ -S "$runtime_dir/ssh-agent" ]; then
              export SSH_AUTH_SOCK="$runtime_dir/ssh-agent"
            else
              mkdir -p "$HOME/.ssh/agent"
              agent_sock="$HOME/.ssh/agent/ssh-agent.sock"
          ...
          # Avoid GUI askpass loops in terminal sessions.
